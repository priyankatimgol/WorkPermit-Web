...
import { template } from './Templetes/DefaultTemplet';
import './style.css';
...
const PrintChecklist = (props) => {
  ...
  return (
    <>
      <div className="container mt-5 mb-5 main-code">
        <div>
        ...
        {sectionlist.map((form, id) => {
          return (
            !loadingData &&
            form?.formData?.map((form, index, array) => {
              const indices = getIndices(index, array.length);
              const isLastItem = index === array.length - 1;

              const pageNumber = indices.current + 1;
              const shouldAddPrintPageClass = pageNumber > 1;
              return (
                <div className="container-fluid datepicker-print">
                  ...
                  {/* {indices.current !== 0 && !isLastItem && (
                    <div
                      className={`page-break ${shouldAddPrintPageClass ? 'printpage' : ''}`}
                    ></div>
                  )} */}
                </div>
              );
